(() => {
    ...
    function initSliders() {
        if (document.querySelector(".template1__slider")) new core(".template1__slider", {
        ...
        if (document.querySelector(".template5__slider")) {
            ...
            function template5SliderHandleMd3Change(e) {
                if (e.matches) {
                    if (!init) {
                        init = true;
                        template5Slider = new core(".template5__slider", {
                            modules: [],
                            observer: true,
                            observeParents: true,
                            slidesPerView: 1.05,
        ...
        if (document.querySelector(".template10__slider")) new core(".template10__slider", {
...
***************************************************************************** */
    ...
    function gsapAnimations() {
        ...
        if (layout3 && window.innerWidth > 767.98) {
            ...
            if (buildingFloors.length > 0) buildingFloors.forEach((element => {
    ...
    if (template6Search && template6Items.length > 0) template6Search.addEventListener("input", (function() {
        ...
        for (var i = 0; i < template6Items.length; i++) {
            ...
            if (-1 !== text.indexOf(query)) itemParent.style.display = "block"; else itemParent.style.display = "none";
        }
    }));
    const tradeArea = document.querySelectorAll(".trade-area");
    if (tradeArea.length > 0) tradeArea.forEach((area => {
        ...
        area.addEventListener("click", (function() {
            ...
            if (storeLogo.textContent) popupMain.querySelector(".store__logo").innerHTML = `\n          <img src="${storeLogo.textContent}" alt="">\n        `;
            if (storeSchedule.textContent) popupMain.querySelector(".store-schedule__body").innerHTML = storeSchedule.outerHTML;
            if (storeFloor.textContent) popupMain.querySelector(".store__location-floor span").innerHTML = storeFloor.textContent;
            if (storeTitle.textContent) popupMain.querySelector(".store__title").innerHTML = storeTitle.textContent;
            if (storeCategory.textContent) popupMain.querySelector(".store__category").innerHTML = storeCategory.textContent;
            if (storeText.textContent) popupMain.querySelector(".store__text").innerHTML = storeText.textContent;
            if (storeLink.textContent) popupMain.querySelector(".store__link").innerHTML = `\n          <a href="${storeLink.textContent}" target="_blank">\n            <i class="link__arrow"></i>\n            <span class="link__text">Подробнее</span>\n          </a>\n        `;
            if (storeGallery.textContent) popupMain.querySelector(".store__gallery").innerHTML = storeGallery.outerHTML;
        }));
    }));
    ...
    if (schemeSearch) schemeSearch.addEventListener("input", (function() {
        var query = this.value.toLowerCase();
        for (var i = 0; i < schemeStores.length; i++) {
            var text = schemeStores[i].textContent.toLowerCase();
            if (-1 !== text.indexOf(query)) schemeStores[i].style.display = "flex"; else schemeStores[i].style.display = "none";
        }
    }));
    ...
    document.addEventListener("afterPopupClose", (function(e) {
        ...
        if ("#store-info" == currentPopup.hash) {
